...
fn wait_ms_util_response(ms:usize) ->bool{
    wait_ms(ms,||{
        let mut raw_int_status_reg = RawInterruptStatusReg::try_from( read_reg(RAW_INT_STATUS_REG)).unwrap();
...
fn send_cmd(cmd_type:Cmd, mut cmd:CmdReg, arg:CmdArg,data_trans_type:DataTransType) ->Option<[u32;4]>{
    ...
    if cmd.data_expected().get_raw()==1{
        ...
        match data_trans_type {
            DataTransType::Read(buffer) => {
                ...
                info!("buf_offset:{}, receive {} bytes",buf_offset,buf_offset*8);
            }
    ...
    let resp = [read_reg(RESP0_REG),read_reg(RESP1_REG),read_reg(RESP2_REG),read_reg(RESP3_REG)];
...
fn test_read(){
    ...
    let _resp = send_cmd(Cmd::ReadSingleBlock,cmd17,arg,DataTransType::Read(&mut buffer)).unwrap();
...
fn test_write_read(){
    ...
    let _resp = send_cmd(Cmd::WriteSingleBlock,cmd24,arg,DataTransType::Write(&buffer)).unwrap();
...
fn check_bus_width(rca:u32)->usize{
    ...
    send_cmd(Cmd::SendScr,acmd51,CmdArg::new(0),DataTransType::Read(&mut buffer));
...
fn check_csd(rca:u32){
    ...
    let resp = send_cmd(Cmd::SendCsd,cmd,CmdArg::new(rca<<16),DataTransType::None).unwrap();
    let status = resp[0];
...
fn check_big_support(sleep:fn(usize))->bool{
 ...
 fn init_sdcard(sleep:fn(usize)){
    ...
    info!("clock_enable: {:#?}", ClockEnableReg::try_from(clock_enable).unwrap());
    // read CARD_TYPE_REG
    let card_type = read_reg(CTYPE_REG);
    info!("card_type: {:#?}", CardTypeReg::try_from(card_type).unwrap());
    ...
    info!("bus_mode(DMA): {:#?}", BusModeReg::try_from(bus_mode).unwrap());
    ...
    info!("clock_divider: {:#?}", ClockDividerReg::try_from(clock_divider).unwrap());
    ...
    info!("CTRL_REG: {:#?}", ControlReg::try_from(read_reg(CTRL_REG)).unwrap());
    let raw_int_status = RawInterruptStatusReg::try_from(read_reg(RAW_INT_STATUS_REG)).unwrap();
